# ASAM OpenSCENARIO® XML

ASAM OpenSCENARIO® **XML** is a distinct standard from ASAM OpenSCENARIO® **DSL**
(see `../asam-openscenario-dsl/`): XML describes concrete scenarios in a declarative
file format, DSL is an abstract scenario description language with its own grammar.

This directory currently carries the normative XML schema only, not the specification
prose.

**These files are not original works of this project.** Copyright © ASAM e.V., 2026.
Redistributed under the ASAM Unrestricted Distribution Clause; see the repository
[LICENSE](../../LICENSE).

## Provenance

| Field | Value |
|---|---|
| Standard | ASAM OpenSCENARIO® XML |
| Version | **V1.4.0** (schema file dated 2026-06-02) |
| Deliverable | `ASAM_OpenSCENARIO_v1.4.0_Schema.zip` |
| Source | <https://publications.pages.asam.net/standards/ASAM_OpenSCENARIO/ASAM_OpenSCENARIO_XML/latest/> |
| Retrieved | 2026-07-28 |
| SHA-256 | `949fe2bcebd1f3fdb941a2cc56641482737ab48e3c5b0eed0ee5294b2355c0e9` |

## Updating

Download the `*_Schema.zip` deliverable from the source URL above, replace
`schema/OpenSCENARIO.xsd`, and update the version, date and checksum here.
