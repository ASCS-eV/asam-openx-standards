# OpenDRIVE — open UML model (SCXML)

This directory holds the **open, tool-neutral UML model** for ASAM OpenDRIVE, so
the ontology/SHACL/XSD pipeline can run **without Enterprise Architect (EA)**.
...
| `opendrive.scxml.zip` | The exact `ModelExport` artifact (`zipOutput=true`); same content as `opendrive.scxml`, directly consumable as a ShapeChange `inputFile`. |
...
Point a ShapeChange OWL (or XSD) configuration's `inputFile` at `opendrive.scxml`
(or `opendrive.scxml.zip`) and set `inputModelType=SCXML`. No EA is required. See
the OMB pipeline docs (`docs/future/asam-openx/`) for the OWL→SHACL steps.
