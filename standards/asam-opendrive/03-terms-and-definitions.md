...
Continuous object
:   An object of continuous, extruded shape.
    Continuous objects are defined using a `<repeat>` element with @distance=0.
...
Origin
:   An element’s (object or signal) origin - or origin point - is the origin of its local coordinate system.
    It defines the position in the road coordinate system where the element is inserted.
...
:   A continuous set of one or more adjacent lane sections on the temporary lane layer.
    It is possible for a road to have no, one, or multiple disjunct temporary layer sections.
